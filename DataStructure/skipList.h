...
protected:
	...
	skipNode<K, E>** last; // last[i] = last node on level i
};

template<class K, class E>
skipList<K, E>::skipList(K largeKey, int maxPairs, float prob)
{// 构造函数, 关键字小于largeKey 且字典数对个数最多为maxPairs, 0 < prob < 1
	cutOff = prob * RAND_MAX;
	maxLevel = (int)ceil(logf(maxPairs) / logf(1 / prob)) - 1; // ceil(log2N) - 1
	levels = 0;
	dSize = 0;
	tailKey = largeKey;
